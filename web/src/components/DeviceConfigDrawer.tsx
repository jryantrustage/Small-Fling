...
import {
  ...
  RefreshCw, Check, Loader2, X, Laptop, Smartphone, Eye,
  Sliders, Key, Cpu, Zap
} from 'lucide-react';
...
export const DeviceConfigDrawer: React.FC<DeviceConfigDrawerProps> = ({
  ...
  const [targetPackage, setTargetPackage] = useState<string>('com.microsoft.teams');
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (data.displays && data.displays.length > 0) {
          ...
          // Set selectedDisplayId to external if available
          const ext = data.displays.find(d => d.isExternal);
          if (ext) {
            setSelectedDisplayId(ext.displayId);
          } else if (data.displays[0]) {
            setSelectedDisplayId(data.displays[0].displayId);
          }
        }
        if (data.kiosk_mode) {
          setKioskMode(data.kiosk_mode);
        }
        if (data.locked_package) {
          setTargetPackage(data.locked_package);
        }
        if (data.peripheral_restrictions) {
          setSuppressHotkeys(data.peripheral_restrictions.suppress_hotkeys ?? true);
          setDisableStatusBar(data.peripheral_restrictions.disable_status_bar ?? true);
          setPreventSleep(data.peripheral_restrictions.prevent_sleep ?? true);
        }
      }
    } catch (e) {
  ...
  const handleExecuteLock = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (data.kiosk_state) {
          setTelemetry(data.kiosk_state);
        }
        if (onShowToast) onShowToast('success', `Kiosk lock engaged on Display #${selectedDisplayId} in ${elapsed}ms ✔`);
      } else {
        const err = await res.json();
        if (onShowToast) onShowToast('error', `Failed to engage lock: ${err.detail || 'Unknown error'}`);
      }
    } catch (e: any) {
      if (onShowToast) onShowToast('error', `Error executing kiosk lock: ${e.message}`);
    } finally {
      setIsLocking(false);
      fetchKioskStatus();
    }
  ...
  // Execute Release Lock (Emergency Exit)
  const handleExecuteRelease = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setAdminPin('');
        if (onShowToast) onShowToast('success', `Kiosk lock safely released in ${elapsed}ms ✔`);
      } else {
        const err = await res.json();
        if (onShowToast) onShowToast('error', `Release error: ${err.detail}`);
      }
    } catch (e: any) {
      if (onShowToast) onShowToast('error', `Error releasing lock: ${e.message}`);
    } finally {
  ...
  const handleProvisionAdmin = async () => {
    ...
    try {
      const res = await fetch(`${apiBase}/api/device/kiosk/provision-admin`, {
        method: 'POST'
      });
      if (res.ok) {
        const d = await res.json();
        setProvisionMessage(d.admin_output || 'Active Admin configured successfully');
        if (onShowToast) onShowToast('success', 'Kiosk Admin provisioning verified');
      }
    } catch (e: any) {
  ...
  const lockStatusBadge = telemetry?.lock_status === 'LOCKED_TASK_EXTERNAL'
    ? { label: 'COSU DEVICE OWNER LOCK', color: '#00ff9d', bg: 'rgba(0, 255, 157, 0.15)' }
    : telemetry?.lock_status === 'PINNED'
    ...
    <>
      ...
      <div className="device-config-drawer">
        ...
        <div className="device-config-header">
          <div className="device-config-header-title">
            <div className="device-config-icon-badge">
              <Shield size={16} color="#00ff9d" />
            </div>
            <div>
              <h3>EXTERNAL KIOSK MANAGER</h3>
              <p>Desktop Display Isolation & Lockdown Control</p>
            </div>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            <button
              type="button"
              className="device-config-refresh-btn"
              onClick={fetchKioskStatus}
              disabled={loading}
            ...
            <button
              type="button"
              className="device-config-close-btn"
              onClick={onClose}
              title="Close Drawer"
            >
              <X size={16} />
        ...
        <div className="device-config-body">
            ...
            <div className="kiosk-status-row">
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                <div
                  ...
                  style={{ background: lockStatusBadge.bg, color: lockStatusBadge.color }}
                >
                  {isLocked ? <Lock size={16} /> : <Unlock size={16} />}
                </div>
                <div>
                  <div className="kiosk-status-pill" style={{ background: lockStatusBadge.bg, color: lockStatusBadge.color }}>
                    <span className="dot" style={{ background: lockStatusBadge.color }} />
                    {lockStatusBadge.label}
                  </div>
                  <div className="kiosk-status-meta">
                    Target Display: <strong>#{selectedDisplayId}</strong> • App: <strong>{telemetry?.locked_package || targetPackage}</strong>
                  </div>
                </div>
              </div>

              {lastActionLatencyMs !== null && (
                <div className="kiosk-latency-badge" title="RPC roundtrip execution latency">
            ...
            <div className="kiosk-displays-list">
              {displays.length > 0 ? (
                ...
                <div className="kiosk-empty-state">
                  ...
                  <span>Scanning connected DisplayPort / Presentation monitors...</span>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="kiosk-section-card">
            ...
            <div className="kiosk-mode-grid">
              <button
                type="button"
                className={`kiosk-mode-tile ${kioskMode === 'freeform' ? 'active' : ''}`}
                onClick={() => setKioskMode('freeform')}
              >
                <div className="mode-tile-header">
                  <Laptop size={15} />
                  <span>Standard Freeform</span>
                </div>
                <p>Standard windowed desktop environment with resizable windows and full taskbar.</p>
              </button>

              <button
                type="button"
                className={`kiosk-mode-tile ${kioskMode === 'mirrored' ? 'active' : ''}`}
                onClick={() => setKioskMode('mirrored')}
              >
                <div className="mode-tile-header">
                  <Eye size={15} />
                  <span>Fullscreen Mirrored</span>
                </div>
                <p>Mirrors the phone screen to external monitor in fullscreen mode.</p>
              </button>

              <button
                type="button"
                className={`kiosk-mode-tile ${kioskMode === 'kiosk' ? 'active' : ''}`}
                onClick={() => setKioskMode('kiosk')}
              >
                <div className="mode-tile-header">
                  <ShieldCheck size={15} color="#00ff9d" />
                  <span>Exclusive Kiosk</span>
                </div>
                <p>Locks target app onto external display. Intercepts Alt+Tab, Meta, and boundary clicks.</p>
              </button>
            </div>
          </div>

          {/* Target Application Setting */}
          <div className="kiosk-section-card">
            <div className="kiosk-section-header">
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            ...
            <div className="kiosk-package-input-group">
              <input
                ...
                placeholder="e.g. com.microsoft.teams or com.matrixcapture.app"
                value={targetPackage}
              ...
              <div className="kiosk-package-pills">
                <button
                  type="button"
                  className={`kiosk-pkg-pill ${targetPackage === 'com.microsoft.teams' ? 'active' : ''}`}
                  onClick={() => setTargetPackage('com.microsoft.teams')}
                >
                  Microsoft Teams
                </button>
                <button
                  type="button"
                  className={`kiosk-pkg-pill ${targetPackage === 'com.matrixcapture.app' ? 'active' : ''}`}
                  onClick={() => setTargetPackage('com.matrixcapture.app')}
                >
                  MatrixCapture Kiosk
                </button>
              </div>
          ...
          {/* Peripheral & Hardware Input Restrictions */}
          <div className="kiosk-section-card">
            <div className="kiosk-section-header">
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            ...
            <div className="kiosk-restrictions-list">
              <label className="kiosk-checkbox-label">
                <input
                  type="checkbox"
                  checked={suppressHotkeys}
                  onChange={(e) => setSuppressHotkeys(e.target.checked)}
                />
                <div className="kiosk-checkbox-text">
                  <span className="kiosk-cb-title">Suppress Global OS Hotkeys</span>
                  <span className="kiosk-cb-desc">Swallow Alt+Tab, Meta / Super, Esc, and App Switch keys inside external display</span>
                </div>
              </label>
              ...
              <label className="kiosk-checkbox-label">
                ...
                />
                <div className="kiosk-checkbox-text">
                  <span className="kiosk-cb-title">Disable Status Bar & Taskbar</span>
                  <span className="kiosk-cb-desc">Hide Android desktop taskbar and immersive system navigation bars</span>
                </div>
              </label>
              ...
              <label className="kiosk-checkbox-label">
                ...
                />
                <div className="kiosk-checkbox-text">
                  <span className="kiosk-cb-title">Prevent Sleep While Docked</span>
                  <span className="kiosk-cb-desc">Enforce FLAG_KEEP_SCREEN_ON and stay awake policy during USB-C DisplayPort docking</span>
                </div>
              </label>
            </div>
          </div>

          {/* Security Context & DPC Info */}
          <div className="kiosk-section-card">
            <div className="kiosk-section-header">
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            ...
            <div className="kiosk-security-details">
              <div className="kiosk-sec-row">
                ...
                <span className={telemetry?.active_admin_active ? 'sec-status-good' : 'sec-status-warn'}>
                  {telemetry?.active_admin_active ? 'ENABLED (KioskAdminReceiver)' : 'NOT CONFIGURED'}
                </span>
              </div>
              <div className="kiosk-sec-row">
                <span>Device Owner (COSU):</span>
                <span className={telemetry?.device_owner_active ? 'sec-status-good' : 'sec-status-info'}>
                  {telemetry?.device_owner_active ? 'ENABLED (Zero-Leakage DPC)' : 'Standard Pinned Lock Task Mode'}
                </span>
              </div>
              ...
              {provisionMessage && (
                <div className="kiosk-provision-msg">{provisionMessage}</div>
              )}
            </div>
          </div>
        ...
        <div className="kiosk-modal-backdrop">
          <div className="kiosk-modal-content">
            <div className="kiosk-modal-header">
              <div className="kiosk-modal-icon-warn">
                <AlertTriangle size={20} color="#f0883e" />
              </div>
              <div>
                <h4>ENGAGE SECURE EXTERNAL KIOSK</h4>
                <p>Hardware Input & Boundary Lockdown Warning</p>
              </div>
            </div>

            <div className="kiosk-modal-body">
              <p>
                Engaging Kiosk Mode will lock <strong>{targetPackage}</strong> onto external Display <strong>#{selectedDisplayId}</strong>:
              </p>
      ...
      {showEmergencyModal && (
        <div className="kiosk-modal-backdrop">
          <div className="kiosk-modal-content">
            <div className="kiosk-modal-header">
              <div className="kiosk-modal-icon-danger">
                <ShieldAlert size={20} color="#f85149" />
              </div>
              <div>
                <h4>EMERGENCY LOCK RELEASE</h4>
                <p>Admin Override to Restore Desktop Freeform Windowing</p>
              </div>
            </div>

            <div className="kiosk-modal-body">
              <p>
                This will terminate Lock Task Mode on Display #{selectedDisplayId}, restore standard freeform desktop window controls, and re-enable global hotkeys.
              </p>
              <div style={{ marginTop: '12px' }}>
                <label style={{ fontSize: '11px', color: 'var(--text-muted)', display: 'block', marginBottom: '4px' }}>
                  Admin Authorization PIN / Confirmation:
                </label>
                <input
                  type="password"
                  className="kiosk-text-input"
                  ...
                  onKeyDown={(e) => {
                    if (e.key === 'Enter') handleExecuteRelease();
                  }}
                />
              </div>
            </div>
